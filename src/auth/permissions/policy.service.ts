 ...
 */
export type PolicyHandler = (permission: PermissionRule | PermissionRule[]) => AsyncRequestHandler;
...
export class PolicyService implements OnModuleInit {
  ...
  private async registerAllPolicy() {
    // project
    await this.registerPolicy('project.create', GeneralPermission(PERMISSION.CREATE_PROJECT, this.prisma));
    await this.registerPolicy('project.view', GeneralPermission(PERMISSION.CREATE_PROJECT, this.prisma));
    await this.registerPolicy('project.edit', GeneralPermission(PERMISSION.CREATE_PROJECT, this.prisma));
    await this.registerPolicy('project.delete', GeneralPermission(PERMISSION.CREATE_PROJECT, this.prisma));
    // permissions
    await this.registerPolicy('permissions.view', ProjectView(this.prisma));
    // attachment
    await this.registerPolicy('projectAttachment.create', GeneralPermission(PERMISSION.CREATE_PROJECT_ATTACHMENT, this.prisma));
    await this.registerPolicy('projectAttachment.view', GeneralPermission(
      [
        PERMISSION.READ_PROJECT_ATTACHMENT_OWN_OR_ALLOWED,
        PERMISSION.READ_PROJECT_ATTACHMENT_NOT_OWN_AND_NOT_ALLOWED,
      ],
      this.prisma));
    await this.registerPolicy('projectAttachment.edit', GeneralPermission(
      [
        PERMISSION.UPDATE_PROJECT_ATTACHMENT_OWN,
        PERMISSION.UPDATE_PROJECT_ATTACHMENT_NOT_OWN,
      ],
      this.prisma));
    await this.registerPolicy('projectAttachment.delete', GeneralPermission(
      [
      ...
      this.prisma));
    // billing account
    await this.registerPolicy('projectBillingAccounts.view', GeneralPermission(
      [
        PERMISSION.READ_AVL_PROJECT_BILLING_ACCOUNTS
      ], 
      this.prisma));
    await this.registerPolicy('projectBillingAccount.view', GeneralPermission(
      [
        PERMISSION.READ_PROJECT_BILLING_ACCOUNT_DETAILS
      ], 
      this.prisma));
    // project member
    await this.registerPolicy('projectMember.create', GeneralPermission(
      [
        PERMISSION.CREATE_PROJECT_MEMBER_OWN,
        PERMISSION.CREATE_PROJECT_MEMBER_NOT_OWN,
      ], 
      this.prisma));
    await this.registerPolicy('projectMember.view', GeneralPermission(
      PERMISSION.READ_PROJECT_MEMBER, 
      this.prisma));
    await this.registerPolicy('projectMember.edit', GeneralPermission(
      [
        PERMISSION.UPDATE_PROJECT_MEMBER_CUSTOMER,
        PERMISSION.UPDATE_PROJECT_MEMBER_NON_CUSTOMER,
      ], 
      this.prisma));
    await this.registerPolicy('projectMember.delete', GeneralPermission(
      [
    ...
    await this.registerPolicy('projectMemberInvite.create', GeneralPermission(
      [
      ...
      this.prisma));
    await this.registerPolicy('projectMemberInvite.view', GeneralPermission(
      [
        PERMISSION.READ_PROJECT_INVITE_OWN,
        PERMISSION.READ_PROJECT_INVITE_NOT_OWN,
      ], 
      this.prisma));
    await this.registerPolicy('projectMemberInvite.edit', GeneralPermission(
      [
        PERMISSION.UPDATE_PROJECT_INVITE_OWN,
        PERMISSION.UPDATE_PROJECT_INVITE_NOT_OWN,
      ], 
      this.prisma));
    await this.registerPolicy('projectMemberInvite.delete', GeneralPermission(
      [
   ...
   */
  private async registerPolicy(policyName: string, handler: AsyncRequestHandler) {
