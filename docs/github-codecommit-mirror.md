...
The [mirror workflow](../.github/workflows/codecommit-mirror.yml) maintains an exact secondary Git copy of `topcoder-platform/projects-api-v6` in the dedicated `projects-api-v6` AWS CodeCommit repository.
...
- CodeCommit repository: `projects-api-v6`
- CodeCommit default branch: `dev`
- IAM role: `GitHubActions-projects-api-v6-CodeCommitMirror`
- Trusted GitHub branches: `dev` and `master`
...
A push to `dev` or `master` starts a full reconciliation. A reconciliation also runs every six hours and can be started manually from either trusted branch.
...
Each run mirrors all current refs. A push to another branch or tag therefore appears in CodeCommit at the next `dev`/`master` push or scheduled reconciliation; workflows on those other refs cannot obtain the AWS role.
...
The `GitHubActions-projects-api-v6-CodeCommitMirror` trust policy accepts only OIDC subjects for this repository's `dev` and `master` branches. It accepts both GitHub's legacy name-based subject and the repository's immutable owner/repository-ID subject. Public forks and workflows on any other upstream ref have different subjects and are denied.
...
Those permissions apply only to `arn:aws:codecommit:us-east-1:811668436784:projects-api-v6`. The role cannot access other CodeCommit repositories, delete repositories, manage IAM, or use other AWS services.
...
- Use **Actions → Mirror GitHub to AWS CodeCommit → Run workflow** from `dev` or `master` for an immediate repair.
...
- Protect `dev` and `master` and require review for changes under `.github/workflows/`.
